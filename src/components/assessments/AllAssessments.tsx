...
type AssessmentStatus = "NOT STARTED" | "IN PROGRESS" | "DONE";
type AssessmentTemplate = "INITIAL ASSESSMENT" | "FOLLOW UP";
...
function AssessmentsPage() {
  const [records, setRecords] = useState(initialAssessments);
  const [selectedAssessmentId, setSelectedAssessmentId] = useState(
    initialAssessments[2]?.id ?? "",
  );
  ...
  const selectedAssessment =
    visibleRecords.find((record) => record.id === selectedAssessmentId) ??
    records.find((record) => record.id === selectedAssessmentId) ??
    visibleRecords[0] ??
    null;
  ...
  function updateAssessment(
    ...
    setRecords((current) =>
      current.map((record) =>
        record.id === assessmentId ? updater(record) : record,
      ),
    );
  }

  function createAssessment() {
    const nextId = `assessment-${records.length + 1}`;
  ...
  function restoreSelectedAssessment() {
    ...
    updateAssessment(selectedAssessment.id, (record) => ({
      ...record,
      deleted: false,
      updatedAt: "Apr 8, 2026 3:23 PM",
    }));
  ...
  function destroySelectedAssessment() {
    if (!selectedAssessment) return;

    const nextRecords = records.filter(
      (record) => record.id !== selectedAssessment.id,
    );
    setRecords(nextRecords);
    setSelectedAssessmentId(nextRecords[0]?.id ?? "");
    setShowOptionsMenu(false);
  }

  function exportSelectedAssessment() {
    if (!selectedAssessment) return;
  ...
  return (
    ...
    >
      <div className="flex h-full gap-2">
          ...
          <div className="min-h-0 overflow-auto">
            <div className="min-w-[980px]">
              ...
              {visibleRecords.map((record) => {
                const isActive = selectedAssessment?.id === record.id;
                  ...
                  >
                    <div className="flex items-center justify-center border-r border-[#f0ece6] py-3">
                      <input
                        ...
                        onChange={(event) => {
                          const checked = event.target.checked;
                          updateAssessment(record.id, (current) => ({
                            ...current,
                            selected: checked,
                          }));
                        }}
        ...
        {showDetailPanel && selectedAssessment ? (
          <aside className="app-panel relative flex w-[340px] flex-col overflow-hidden rounded-2xl">
            ...
            {selectedAssessment.deleted ? (
              <div className="flex items-center justify-between bg-[#eb4d4d] px-4 py-2.5 text-[14px] font-medium text-white">
                ...
                <button
                  type="button"
                  onClick={restoreSelectedAssessment}
            ...
            <div className="flex-1 overflow-auto">
              <div className="grid grid-cols-2 gap-x-5 gap-y-4 border-b border-[#f0ece6] px-4 py-4 text-[13px]">
                ...
                <div>
                  <AvatarPill name={selectedAssessment.createdBy} />
                </div>
                ...
                <div>
                  <AvatarPill name={selectedAssessment.updatedBy} />
                </div>
              </div>
              ...
              <div className="relative" ref={menuRef}>
                ...
                {showOptionsMenu ? (
                  <div className="absolute bottom-[calc(100%+8px)] left-0 w-[205px] rounded-xl border border-[#ece8e1] bg-white p-2 shadow-[0_8px_32px_rgba(15,23,42,0.12)]">
                    ...
                    <button
                      type="button"
                      onClick={destroySelectedAssessment}
                      className="mt-1 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[14px] text-slate-500 hover:bg-[#f7f5f1]"
                    >
                      <Trash2 className="h-4 w-4" />
                    ...
                    <button
                      type="button"
                      onClick={exportSelectedAssessment}
                      className="mt-1 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[14px] text-slate-500 hover:bg-[#f7f5f1]"
                    >
