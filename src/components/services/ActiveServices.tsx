...
function ActiveServicePage() {
  const [rows, setRows] = useState<ActiveServiceRow[]>(
    Array.from({ length: 1 }, (_, index) => ({
      id: `pending-${index + 1}`,
    })),
  );
  const [filterCount, setFilterCount] = useState(0);
  const [viewName, setViewName] = useState("Service Catalog");
  const [selectedIds, setSelectedIds] = useState<Record<string, boolean>>({});

  const columns = useMemo<ColumnDef<ActiveServiceRow>[]>(
    () => [
      {
        id: "select",
        header: () => (
  ...
  function resetView() {
    setFilterCount(0);
    setSelectedIds({});
    setViewName("Active Service");
  }

  function updateView() {
    setViewName((current) =>
      current === "Active Service"
        ? "Active Service (Updated)"
        : "Active Service",
    );
  }
