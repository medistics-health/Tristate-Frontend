...
const statusBadgeClassNames: Record<PendingSignatresStatus, string> = {
...
const initialRecords: PendingSignatres[] = Array.from(
  { length: 8 },
  (_, index) => ({
...
function formatStatusLabel(status: PendingSignatresStatus) {
...
function AgreementPendingSignaturesPage() {
  ...
  function upsertSelectedAudit(
    auditId: string,
    updater: (record: PendingSignatres) => PendingSignatres,
  ) {
    setRecords((current) =>
      current.map((record) =>
  ...
  function createAudit() {
    ...
    const newRecord: PendingSignatres = {
      id: nextId,
      title: "",
      type: "COMPLIANCE",
      createdBy: "Siddhi Gajjar",
  ...
  return (
    <AppLayout
      ...
      activeSubItem="Pending Sinatures"
