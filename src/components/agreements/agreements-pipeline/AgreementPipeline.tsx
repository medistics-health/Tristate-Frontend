...
type AgreementPiplineProgres = {
  id: AgreementPipelinetatus;
  label: string;
  badgeClassName: string;
};

const AgreementPipeline: AgreementPiplineProgres[] = [
  {
    id: "scheduled",
    label: "SCHEDULED",
...
const initialCards: AgreementPipelineCard[] = [
  {
    id: "audit-1",
    title: "Untitled",
    status: "scheduled",
...
function AgreementPipelinePage() {
  ...
  const visibleLanes = useMemo(
    () =>
      hideClosed
        ? AgreementPipeline.filter((lane) => lane.id !== "closed")
        : AgreementPipeline,
    [hideClosed],
  );
  ...
  const cardsByLane = useMemo(
    () =>
      ...
      ) as Record<AgreementPipelinetatus, AgreementPipelineCard[]>,
    [cards, visibleLanes],
  );
  ...
  function addAudit(status: AgreementPipelinetatus) {
    const nextId = `audit-${cards.length + 1}`;
    const newCard: AgreementPipelineCard = {
      id: nextId,
      title: "Untitled",
      status,
      type: "COMPLIANCE",
