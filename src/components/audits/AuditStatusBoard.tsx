...
function AuditStatusBoard() {
  ...
  const [selectedAuditId, setSelectedAuditId] = useState(initialCards[4]?.id ?? "");
  const [activeTab, setActiveTab] = useState<(typeof detailTabs)[number]["id"]>("home");
  const [hideClosed, setHideClosed] = useState(false);
  const [showDetailPanel, setShowDetailPanel] = useState(true);

  const visibleLanes = useMemo(
    () => (hideClosed ? auditLanes.filter((lane) => lane.id !== "closed") : auditLanes),
    [hideClosed],
  );
  ...
  const selectedAudit =
    ...
    cards.find((card) => visibleLanes.some((lane) => lane.id === card.status)) ??
    null;
  ...
  function sortLanesByCount() {
    setCards((current) =>
      [...current].sort((left, right) => left.status.localeCompare(right.status)),
    );
  }
        ...
        <div className="app-panel min-w-0 flex-1 overflow-hidden rounded-2xl">
          <div className="flex items-center justify-between border-b border-[#f0ece6] px-4 py-2.5">
            ...
            <div className="flex items-center gap-6 text-[14px] text-slate-500">
              <button type="button" onClick={() => setHideClosed((current) => !current)}>
                Filter
              </button>
