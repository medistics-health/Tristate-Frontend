...
function PendingApprovalPage() {
  ...
  const [viewName, setViewName] = useState("Pending Approval");
  const [selectedIds, setSelectedIds] = useState<Record<string, boolean>>({});

  const columns = useMemo<ColumnDef<PendingApprovalRow>[]>(
    () => [
      {
        id: "select",
        header: () => (
  ...
  function resetView() {
    setFilterCount(0);
    setSelectedIds({});
    setViewName("Pending Approval");
  }

  function updateView() {
    setViewName((current) =>
      current === "Pending Approval"
        ? "Pending Approval (Updated)"
        : "Pending Approval",
    );
  }
