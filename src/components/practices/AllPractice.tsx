...
type PracticePartnerView = {
  id: string;
  label: string;
  sort: SortingState;
};

const initialRows: PracticePartnerRow[] = [];

const views: PracticePartnerView[] = [
  {
    id: "all",
    label: "All Practice Partner",
...
function AllPracticePage() {
  ...
  const columns = useMemo<ColumnDef<PracticePartnerRow>[]>(
    () => [
      {
        id: "select",
        header: () => (
  ...
  function addPracticePartner() {
    const nextId = `po-${rows.length + 1}`;
    setRows((current) => [
      {
        id: nextId,
        name: `Practice Partner ${current.length + 1}`,
