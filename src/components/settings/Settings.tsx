...
import { getAllUsers, updateUserApi, createUserApi, deleteUserApi, getSystemSettingsApi, updateSystemSettingsApi } from "../../services/operations/users";
import { getQuickBooksStatus, disconnectQuickBooks, connectQuickBooks } from "../../services/operations/quickbooks";
...
export default function SettingsPage() {
  ...
  async function handleCreateUser(e: React.FormEvent) {
    ...
    try {
      ...
      setNewUser({ userName: "", firstName: "", lastName: "", email: "", password: "", role: "VIEWER" });
      loadUsers();
    } catch (e) {
      ...
      activeSubItem={
        activeTab === "general" ? "General Settings" :
          activeTab === "integrations" ? "API & Integrations" :
            activeTab === "team" ? "Team Management" : "Security & Access"
      }
    >
        ...
        <div className="flex items-center justify-between px-2 py-1">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white border border-[#ece8e1] shadow-sm">
              {activeTab === "general" && <SettingsIcon className="h-6 w-6 text-[#4f63ea]" />}
              {activeTab === "integrations" && <LinkIcon className="h-6 w-6 text-[#4f63ea]" />}
              {activeTab === "team" && <User className="h-6 w-6 text-[#4f63ea]" />}
              {activeTab === "security" && <Shield className="h-6 w-6 text-[#4f63ea]" />}
            </div>
            <div>
              <h1 className="text-2xl font-bold text-slate-800 capitalize leading-tight">
                {activeTab === "integrations" ? "API & Integrations" : activeTab.replace(/-/g, " ")}
              </h1>
          ...
          <div className="flex items-center gap-3">
            {activeTab === "team" && (
              <button onClick={() => setShowAddModal(true)} className="rounded-xl bg-[#4f63ea] px-5 py-2.5 text-sm font-bold text-white transition-all hover:bg-[#3d4ed1] shadow-lg shadow-blue-500/10">
                Add Member
              </button>
            )}
            <button
        ...
        <div className="flex-1 overflow-y-auto rounded-3xl border border-[#ece8e1] bg-white p-5 shadow-sm m-2">
          ...
          {activeTab === "general" && (
            <div className="max-w-2xl space-y-8">
              {isLoadingSettings ? (
                <div className="flex justify-center py-20"><div className="h-8 w-8 animate-spin rounded-full border-4 border-[#ece8e1] border-t-[#4f63ea]" /></div>
              ) : (
                <section>
                  <h3 className="text-lg font-bold text-slate-800">Organization Profile</h3>
                  <p className="text-sm text-slate-500 mb-6">This information will be displayed on invoices and reports.</p>
          ...
          {activeTab === "integrations" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="overflow-hidden rounded-2xl border border-[#ece8e1] bg-white shadow-sm transition-all hover:shadow-md">
                <div className="border-b border-[#f1efeb] bg-[#fbfaf8] p-5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h3 className="text-[15px] font-bold text-slate-800">QuickBooks Online</h3>
                        <p className="text-xs text-slate-500">Manage automated accounting sync for all your practices.</p>
                      </div>
                    </div>
                  ...
                  <div className="max-h-[500px] overflow-y-auto">
                    <table className="w-full text-left text-sm">
                      <thead className="sticky top-0 bg-white border-b border-[#ece8e1] z-10">
                        <tr>
                          <th className="px-5 py-3 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Practice / Company</th>
                          <th className="px-5 py-3 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Sync Status</th>
                          <th className="px-5 py-3 text-right font-bold text-slate-400 uppercase tracking-widest text-[10px]">Action</th>
                        </tr>
                      </thead>
                            ...
                            <tr key={company.id} className="hover:bg-slate-50/50 transition-colors group">
                              <td className="px-5 py-4">
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className="font-bold text-slate-700 text-[13px]">{company.name}</span>
                                </div>
                              </td>
                              <td className="px-5 py-4">
                                {isConnected ? (
              ...
              <div className="flex items-center justify-between rounded-2xl border border-[#ece8e1] bg-white p-5 shadow-sm transition-all hover:shadow-md">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h4 className="text-[15px] font-bold text-slate-800">Stripe Payments</h4>
                    <p className="text-xs text-slate-500">Handle credit cards & ACH processing for your invoices.</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-[#ece8e1] bg-white p-5 shadow-sm transition-all hover:shadow-md">
                ...
                {isMercuryConnected === null ? (
                  ...
                  <span className="flex items-center gap-1.5 rounded-full bg-slate-50 px-4 py-1.5 text-xs font-bold text-slate-500 border border-slate-200 shadow-sm">
                    <div className="h-1.5 w-1.5 rounded-full bg-slate-400" /> Disconnected
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-8">
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="text-sm font-medium text-slate-400">
                  Total: <span className="text-slate-800 font-bold">{users.length}</span>
                </div>
              </div>
              ...
              {isLoadingUsers ? (
                <div className="flex flex-col items-center justify-center py-24 gap-4">
                  ...
                  <p className="text-sm font-medium text-slate-500">Fetching team members...</p>
                </div>
              ) : (
                <div className="overflow-hidden rounded-2xl border border-[#ece8e1]">
                  <table className="w-full text-left text-sm">
                    <thead className="bg-[#fbfaf8] border-b border-[#ece8e1]">
                      <tr>
                        <th className="px-6 py-5 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Team Member</th>
                        <th className="px-6 py-5 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Access Role</th>
                        <th className="px-6 py-5 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Account Status</th>
                        <th className="px-6 py-5 text-right font-bold text-slate-400 uppercase tracking-widest text-[10px]">Action</th>
                      </tr>
                    </thead>
                        ...
                        <tr key={user.id} className="hover:bg-blue-50/30 transition-all group">
                          ...
                          <td className="px-6 py-5 text-right">
                            <div className="flex justify-end gap-1">
                              <button onClick={() => setEditingUser(user)} className="p-2.5 rounded-xl text-slate-400 hover:bg-white hover:text-[#4f63ea] hover:shadow-sm transition-all"><Pencil className="h-4 w-4" /></button>
                              <button onClick={() => handleDeleteUser(user.id)} className="p-2.5 rounded-xl text-slate-400 hover:bg-white hover:text-red-500 hover:shadow-sm transition-all"><Trash2 className="h-4 w-4" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {activeTab === "security" && (
            <div className="max-w-xl space-y-6">
              <div className="rounded-2xl border border-[#ece8e1] bg-[#fbfaf8] p-6">
                ...
                <p className="text-sm text-slate-500">Secure your account by requiring an additional verification code.</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between border-b border-[#f1efeb] px-8 py-6">
              ...
              <button onClick={() => setEditingUser(null)} className="text-slate-400 hover:text-slate-600"><X className="h-6 w-6" /></button>
            </div>
            <form onSubmit={handleUpdateUser} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">First Name</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Last Name</label>
                  <input
                    type="text"
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setEditingUser(null)} className="flex-1 rounded-xl border border-[#ece8e1] py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
                <button type="submit" className="flex-1 rounded-xl bg-[#4f63ea] py-3 text-sm font-bold text-white hover:bg-[#3d4ed1] transition-all shadow-lg shadow-blue-500/20">Update User</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleCreateUser} className="p-8 space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 rounded-xl border border-[#ece8e1] py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
                <button type="submit" className="flex-1 rounded-xl bg-[#4f63ea] py-3 text-sm font-bold text-white hover:bg-[#3d4ed1] transition-all shadow-lg shadow-blue-500/20">Add Member</button>
              </div>
            </form>
          </div>
        </div>
