...
import type { CompanyCellValue, CompanyRow, CompanyUserValue, CompanyViewData } from "./types";
import { createCompanyApi, deleteCompanyApi, getCompaniesView, updateCompanyApi } from "../../services/operations/companies";
import toast from "react-hot-toast";

function isUserValue(value: CompanyCellValue): value is CompanyUserValue {
  return typeof value === "object" && value !== null && "name" in value && "initials" in value;
}
...
export default function AllCompaniesPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setIsLoading(true);
        setError(null);
        const data = await getCompaniesView();
        setViewData(data);
        setRows(data.rows);
    ...
    const cols: ColumnDef<CompanyRow>[] = [
      ...
      ...visibleFields.map((field) => {
        const iconMap: Record<string, React.ReactNode> = {
          ...
          lastUpdate: <SlidersHorizontal className="h-3.5 w-3.5 text-slate-400" />,
        ...
        return {
          id: field.id,
          accessorFn: (row: CompanyRow) => getCellDisplayValue(row.values[field.id]),
          header: () => (
            <div className="flex items-center gap-2">
              {iconMap[field.id] || <FileText className="h-3.5 w-3.5 text-slate-400" />}
          ...
          cell: ({ row }: { row: { original: CompanyRow } }) => {
            ...
            if (field.id === "status") {
              ...
              return (
                <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${statusColors[String(value)] || ""}`}>
                  {String(value)}
                </span>
              );
            }
            if (field.id === "size" || field.id === "revenue" || field.id === "practicesCount") {
            ...
            if (field.id === "website" && value) {
              return (
                <a href={String(value)} target="_blank" rel="noopener noreferrer" className="text-[#4f63ea] hover:underline">
                  {String(value)}
                </a>
              );
            }
  ...
  async function handleUpdateCompany(e: React.FormEvent) {
    ...
    try {
      ...
      const updated = await updateCompanyApi(selectedRow.id, companyData);
      setRows((current) => current.map((row) => (row.id === selectedRow.id ? updated : row)));
      setIsEditing(false);
  ...
  const views = [
    { id: "all", label: "All Companies", sort: [{ id: "creationDate", desc: true }] as SortingState },
    { id: "recent", label: "Recently Updated", sort: [{ id: "lastUpdate", desc: true }] as SortingState },
  ];
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-[13px] font-medium text-slate-700">Company Details</h3>
          <button
            type="button"
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-[13px]">
            ...
            <span className="font-medium text-slate-700">{String(values.name || "-")}</span>
          </div>

          <div className="flex items-center gap-2 text-[13px]">
            <span className="w-24 text-slate-400">Status:</span>
            <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${
              String(values.status) === "ACTIVE" ? "bg-green-100 text-green-700" :
              String(values.status) === "PROSPECT" ? "bg-blue-100 text-blue-700" :
              String(values.status) === "LEAD" ? "bg-yellow-100 text-yellow-700" :
          ...
          <div className="flex items-center gap-2 text-[13px]">
            ...
            <span className="text-slate-700">{String(values.domain || "-")}</span>
          </div>

          <div className="flex items-center gap-2 text-[13px]">
            <span className="w-24 text-slate-400">Industry:</span>
            <span className="text-slate-700">{String(values.industry || "-")}</span>
          </div>

          <div className="flex items-center gap-2 text-[13px]">
            <span className="w-24 text-slate-400">Size:</span>
            <span className="text-slate-700">{Number(values.size || 0).toLocaleString()}</span>
          </div>

          <div className="flex items-center gap-2 text-[13px]">
            <span className="w-24 text-slate-400">Revenue:</span>
            <span className="text-slate-700">${Number(values.revenue || 0).toLocaleString()}</span>
          </div>

          <div className="flex items-center gap-2 text-[13px]">
            <span className="w-24 text-slate-400">Email:</span>
            <span className="text-slate-700">{String(values.email || "-")}</span>
          </div>

          <div className="flex items-center gap-2 text-[13px]">
            <span className="w-24 text-slate-400">Phone:</span>
            <span className="text-slate-700">{String(values.phone || "-")}</span>
          </div>

          <div className="flex items-center gap-2 text-[13px]">
            <span className="w-24 text-slate-400">Website:</span>
            {values.website ? (
              <a href={String(values.website)} target="_blank" rel="noopener noreferrer" className="text-[#4f63ea] hover:underline">
        ...
        <div className="border-t border-[#f0ece6] pt-4">
          <h4 className="mb-3 text-[13px] font-medium text-slate-700">Address</h4>
          <div className="space-y-2 text-[13px]">
            <div className="flex items-center gap-2">
              <span className="w-24 text-slate-400">Street:</span>
              <span className="text-slate-700">{String(values.street || "-")}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-24 text-slate-400">City:</span>
              <span className="text-slate-700">{String(values.city || "-")}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-24 text-slate-400">State:</span>
              <span className="text-slate-700">{String(values.state || "-")}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-24 text-slate-400">ZIP:</span>
              <span className="text-slate-700">{String(values.zip || "-")}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-24 text-slate-400">Country:</span>
              <span className="text-slate-700">{String(values.country || "-")}</span>
            </div>
          </div>
        </div>
    ...
    return (
      <form onSubmit={handleUpdateCompany} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-[12px] font-medium text-slate-600">Domain</label>
            <input
              type="text"
          ...
          <div>
            <label className="mb-1 block text-[12px] font-medium text-slate-600">Industry</label>
            <input
              type="text"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-[12px] font-medium text-slate-600">Size</label>
            <input
              type="number"
              value={formData.size}
          ...
          <div>
            <label className="mb-1 block text-[12px] font-medium text-slate-600">Revenue</label>
            <input
              type="number"
        ...
        <div>
          <label className="mb-1 block text-[12px] font-medium text-slate-600">Website</label>
          <input
            type="url"
        ...
        <div className="border-t border-[#f0ece6] pt-4">
          <h4 className="mb-3 text-[12px] font-medium text-slate-600">Address</h4>
          <div className="space-y-3">
            <div>
              <label className="mb-1 block text-[11px] text-slate-500">Street</label>
              <input
                type="text"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1 block text-[11px] text-slate-500">City</label>
                <input
                  type="text"
                  value={formData.city}
              ...
              <div>
                <label className="mb-1 block text-[11px] text-slate-500">State</label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1 block text-[11px] text-slate-500">ZIP</label>
                <input
                  type="text"
                  value={formData.zip}
              ...
              <div>
                <label className="mb-1 block text-[11px] text-slate-500">Country</label>
                <input
                  type="text"
  ...
  if (isLoading) {
    return (
      <AppLayout title="Companies" activeModule="Companies" activeSubItem="All Companies">
            ...
            {rows.length === 0 ? (
              <div className="relative flex min-h-[520px] items-center justify-center">
                ...
                <div className="flex max-w-md flex-col items-center px-6 text-center">
                  ...
                  <h2 className="mt-4 text-[15px] font-semibold text-slate-700">Add your first Company</h2>
                  <p className="mt-2 text-[14px] text-slate-400">Use our API or add your first Company manually</p>
              ...
              <div className="border-b border-[#f4f1ec] px-4 py-2 text-[13px] text-slate-400">
                <button type="button" onClick={openCreateForm} className="inline-flex items-center gap-2">
                  <Plus className="h-3.5 w-3.5" />
                  Add New
                </button>
              </div>
        ...
        {showCreateForm && (
          <aside className="app-panel flex w-[400px] flex-col overflow-hidden rounded-2xl border border-[#f0ece6] bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-[#f0ece6] px-4 py-3">
              <h2 className="text-[15px] font-semibold text-slate-700">Create Company</h2>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateCompany} className="flex-1 overflow-auto p-4">
              <div className="space-y-4">
                <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="mb-1 block text-[13px] font-medium text-slate-700">Domain</label>
                    <input
                      type="text"
                      value={formData.domain}
                      onChange={(e) => handleFormChange("domain", e.target.value)}
                  ...
                  <div>
                    <label className="mb-1 block text-[13px] font-medium text-slate-700">Industry</label>
                    <input
                      type="text"
                      value={formData.industry}
                      onChange={(e) => handleFormChange("industry", e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="mb-1 block text-[13px] font-medium text-slate-700">Size</label>
                    <input
                      type="number"
                      value={formData.size}
                      onChange={(e) => handleFormChange("size", e.target.value)}
                  ...
                  <div>
                    <label className="mb-1 block text-[13px] font-medium text-slate-700">Revenue</label>
                    <input
                      type="number"
                      value={formData.revenue}
                      onChange={(e) => handleFormChange("revenue", e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="mb-1 block text-[13px] font-medium text-slate-700">Phone</label>
                    <input
                      type="text"
                      value={formData.phone}
                      onChange={(e) => handleFormChange("phone", e.target.value)}
                ...
                <div>
                  <label className="mb-1 block text-[13px] font-medium text-slate-700">Website</label>
                  <input
                    type="url"
                    value={formData.website}
                    onChange={(e) => handleFormChange("website", e.target.value)}
                ...
                <div className="border-t border-[#f0ece6] pt-4">
                  <h3 className="mb-3 text-[13px] font-medium text-slate-700">Address</h3>
                  <div className="space-y-3">
                    <div>
                      <label className="mb-1 block text-[12px] text-slate-500">Street</label>
                      <input
                        type="text"
                        value={formData.street}
                        onChange={(e) => handleFormChange("street", e.target.value)}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="mb-1 block text-[12px] text-slate-500">City</label>
                        <input
                          type="text"
                          value={formData.city}
                          onChange={(e) => handleFormChange("city", e.target.value)}
                      ...
                      <div>
                        <label className="mb-1 block text-[12px] text-slate-500">State</label>
                        <input
                          type="text"
                          value={formData.state}
                          onChange={(e) => handleFormChange("state", e.target.value)}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="mb-1 block text-[12px] text-slate-500">ZIP Code</label>
                        <input
                          type="text"
                          value={formData.zip}
                          onChange={(e) => handleFormChange("zip", e.target.value)}
                      ...
                      <div>
                        <label className="mb-1 block text-[12px] text-slate-500">Country</label>
                        <input
                          type="text"
                          value={formData.country}
                          onChange={(e) => handleFormChange("country", e.target.value)}
