...
type OnboardingCredentialingExtended = OnboardingCredentialing & {
  payerPortalLogins?: PayerPortalLogin[];
};

const initialPayerPortalLogin: PayerPortalLogin = {
  payerName: "",
  portalUrl: "",
  username: "",
  password: "",
...
const createInitialPayerPortalLogins = (count = 10): PayerPortalLogin[] =>
  Array.from({ length: count }, () => ({ ...initialPayerPortalLogin }));
...
const payerPortalStatusOptions: Option[] = [
...
const documentTypeOptions: Option[] = [
  { label: "W-9", value: "W9" },
  { label: "Void Check", value: "VOID_CHECK" },
  { label: "Bank Account Letter", value: "BANK_ACCOUNT_LETTER" },
  { label: "IRS Letter 147C", value: "IRS_LETTER_147C" },
  { label: "In-Network Insurance Roster", value: "IN_NETWORK_INSURANCE_ROSTER" },
  { label: "Insurance Enrollment List", value: "INSURANCE_ENROLLMENT_LIST" },
  { label: "Board Certification", value: "BOARD_CERTIFICATION" },
  { label: "Professional Liability Insurance (PLI)", value: "PLI" },
  { label: "Driver License", value: "DRIVER_LICENSE" },
  { label: "Social Security Card", value: "SOCIAL_SECURITY_CARD" },
  { label: "Passport-sized Photo", value: "PASSPORT_SIZED_PHOTO" },
...
const credentialingDocumentChecklist: Array<{
  label: string;
  value: string;
  scope: "provider" | "practice";
}> = [
  {
  ...
  {
    label: "Insurance Enrollment List",
    value: "INSURANCE_ENROLLMENT_LIST",
    scope: "provider",
  },
  {
    label: "Copy of Bachelor Degree",
    value: "BACHELOR_DEGREE",
    scope: "provider",
  },
  {
    label: "Copy of Master Degree",
    value: "MASTER_DEGREE",
    scope: "provider",
  },
  {
    label: "IRS Letter 147C",
    value: "IRS_LETTER_147C",
    scope: "practice",
  },
  {
    label: "In-Network Insurance Roster",
    value: "IN_NETWORK_INSURANCE_ROSTER",
    scope: "practice",
  },
];
...
const initialCredentialing: OnboardingCredentialingExtended = {
  ...
  additionalNotes: "",
  payerPortalLogins: createInitialPayerPortalLogins(),
};
...
const initialFormData: OnboardingBody = {
  ...
  credentialing: { ...initialCredentialing } as OnboardingCredentialing,
...
function normalizeLoadedOnboarding(onboarding: Onboarding): OnboardingBody {
  return {
    ...
    credentialing: {
      ...initialFormData.credentialing,
      ...(onboarding.credentialing ?? {}),
      payerPortalLogins:
        (
          onboarding.credentialing as
            | OnboardingCredentialingExtended
            | undefined
        )?.payerPortalLogins?.length
          ? (
              onboarding.credentialing as OnboardingCredentialingExtended
            ).payerPortalLogins
          : createInitialPayerPortalLogins(),
    } as OnboardingCredentialing,
...
export default function OnboardingFormV5() {
  ...
  function applyCompanyInfoToPracticeOne(
    prev: OnboardingBody,
  ): OnboardingBody {
  ...
  function applyCompanyAddressToPrimaryLocation(
    prev: OnboardingBody,
  ): OnboardingBody {
    const practices = (prev.practices ?? []).slice();
    const currentPractice = practices[0] ?? { ...initialPractice };
  ...
  function getPayerPortalLogins(): PayerPortalLogin[] {
    const credentialing = formData.credentialing as
      | OnboardingCredentialingExtended
      | undefined;
    const logins = credentialing?.payerPortalLogins;
    if (logins && logins.length > 0) return logins;
    return createInitialPayerPortalLogins();
  }

  function updatePayerPortalLogin<K extends keyof PayerPortalLogin>(
    index: number,
    field: K,
    value: PayerPortalLogin[K],
  ) {
    setFormData((prev) => {
      const credentialing = (prev.credentialing ??
        initialCredentialing) as OnboardingCredentialingExtended;
      const currentLogins =
        credentialing.payerPortalLogins &&
        credentialing.payerPortalLogins.length > 0
          ? credentialing.payerPortalLogins
          : createInitialPayerPortalLogins();
      const nextLogins = currentLogins.map((login, loginIndex) =>
        loginIndex === index ? { ...login, [field]: value } : login,
      );
      return {
        ...prev,
        credentialing: {
          ...credentialing,
          payerPortalLogins: nextLogins,
        },
      };
    });
  }

  function addPayerPortalLogin() {
    setFormData((prev) => {
      const credentialing = (prev.credentialing ??
        initialCredentialing) as OnboardingCredentialingExtended;
      const currentLogins =
  ...
  function removePayerPortalLogin(index: number) {
    setFormData((prev) => {
      const credentialing = (prev.credentialing ??
        initialCredentialing) as OnboardingCredentialingExtended;
      const currentLogins = credentialing.payerPortalLogins ?? [];
      if (currentLogins.length <= 1) return prev;
      return {
        ...prev,
        credentialing: {
          ...credentialing,
          payerPortalLogins: currentLogins.filter(
    ...
    <Shell>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <label className="flex items-start gap-3 text-sm text-slate-700">
                              <input
                                ...
                                onChange={(event) => {
                                  ...
                                  setCopyCompanyInfoToPracticeOne(checked);
                                  if (checked) {
                                    setCopyCompanyAddressToPrimaryLocation(
                                      true,
                                    );
                                    setFormData((prev) =>
                                      applyCompanyInfoToPracticeOne(prev),
                                    );
                                  }
                                }}
                              />
                              <span>
                                ...
                                <span className="mt-1 block text-xs text-slate-500">
                                  ...
                                  location. You can still edit the fields after
                                  copying.
                                </span>
                              </span>
                            </label>
                          ...
                          <div className="lg:col-span-3">
                            <Field label="IPA Affiliations">
                              <TextArea
                                ...
                                placeholder="List all Independent Practice Association affiliations"
                              />
                            </Field>
                          </div>
                          ...
                          <div className="lg:col-span-3">
                            <Field label="Billing Address" required>
                              <TextArea
                                ...
                                placeholder="Address for billing and claims correspondence"
                              />
                            </Field>
                          </div>
                          ...
                          <div className="lg:col-span-3">
                            <Field label="Mailing Address">
                              <TextArea
                                ...
                                placeholder="Mailing address if different from billing address"
                              />
                            </Field>
                          </div>
                                        ...
                                        <label className="flex items-start gap-3 text-sm text-slate-700">
                                          <input
                                            ...
                                            onChange={(event) => {
                                              ...
                                              if (checked) {
                                                setFormData((prev) =>
                                                  applyCompanyAddressToPrimaryLocation(
                                                    prev,
                                                  ),
                                                );
                                              }
                                          ...
                                          <span>
                                            ...
                                            <span className="mt-1 block text-xs text-slate-500">
                                              ...
                                              this location. You can still edit
                                              the fields after copying.
                                            </span>
                                          </span>
                                        </label>
                                      </div>
              ...
              <SectionCard title="Credentialing / Payer Enrollment">
                <div className="grid gap-6">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                    <RepeaterHeader
                      title="Payer Portal Login Credentials"
                      actionLabel="+ Add Portal Login"
                      onAction={addPayerPortalLogin}
                    />
                    <p className="mb-4 text-sm text-slate-500">
                      Capture payer portal URLs and login details. Start with 10
                      rows and add more as needed.
                    </p>
                    <div className="space-y-4">
                      {getPayerPortalLogins().map((portal, portalIndex) => (
                        <div
                          key={`payer-portal-${portalIndex}`}
                          className="rounded-2xl border border-slate-200 bg-white p-4"
                        >
                          <div className="mb-4 flex items-center justify-between gap-4">
                            <p className="font-medium text-slate-800">
                              Portal Login {portalIndex + 1}
                            </p>
                            {getPayerPortalLogins().length > 1 ? (
                              <button
                                type="button"
                                onClick={() =>
                                  removePayerPortalLogin(portalIndex)
                                }
                                className="text-sm text-red-500"
                              >
                                Remove
                          ...
                          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                            <Field label="Payer / Insurance Company">
                              <TextInput
                                value={portal.payerName ?? ""}
                                onChange={(event) =>
                                  updatePayerPortalLogin(
                                    portalIndex,
                                    "payerName",
                                    event.target.value,
                                  )
                            ...
                            <Field label="Portal URL / Website">
                              <TextInput
                                type="url"
                                placeholder="https://"
                            ...
                            <Field label="Designated Contact Name">
                              <TextInput
                                value={portal.designatedContactName ?? ""}
                                onChange={(event) =>
                                  updatePayerPortalLogin(
                                    portalIndex,
                                    "designatedContactName",
                            ...
                            <Field label="Status">
                              <SelectInput
                                value={portal.status ?? "NOT_STARTED"}
                                onChange={(event) =>
                                  updatePayerPortalLogin(
                                    portalIndex,
                                    "status",
                                ...
                                options={payerPortalStatusOptions}
                              />
                            </Field>
        ...
        {currentStep === 8 ? (
          <>
            <SectionCard
              title="Credentialing Document Checklist"
              description="Track the DocPro credentialing documents. Provider-level uploads live on the Providers step; practice-level uploads live under Operations. Use the tracker below for any remaining checklist items."
            >
              <div className="space-y-3">
                {credentialingDocumentChecklist.map((item) => {
                  const tracked = (formData.documents ?? []).find((document) => {
                    const type = Array.isArray(document.documentType)
                      ? document.documentType[0]
                      : document.documentType;
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {item.scope === "provider"
                            ...
                            : "Practice-level"}
                          {tracked?.status
                            ? ` · Tracker status: ${tracked.status.replaceAll("_", " ")}`
                            : ""}
                        </p>
                      </div>
                      <button
                        ...
                        onClick={() => {
                          const alreadyTracked = (formData.documents ?? []).some(
                            (document) => {
                              const type = Array.isArray(document.documentType)
                                ? document.documentType[0]
                                : document.documentType;
                          ...
                          setFormData((prev) => ({
                            ...
                            documents: [
                              ...
                              {
                                ...initialDocument,
                                documentType: item.value as unknown as OnboardingDocument["documentType"],
                                required: false,
                                status: "NOT_REQUESTED",
                              },
                            ],
                      ...
                      >
                        {tracked ? "Already in tracker" : "+ Add to tracker"}
                      </button>
                    </div>
                  );
                })}
