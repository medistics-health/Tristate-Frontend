...
const detailTabs = [
  { id: "home", label: "Home", icon: <Circle className="h-4 w-4" /> },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <button
            ...
            className={`inline-flex items-center rounded-md px-2 py-0.5 text-[12px] font-medium transition-colors hover:opacity-80 ${row.original.titleAccent}`}
          >
            {row.original.title}
          </button>
        ),
          ...
          <div className="flex items-center justify-between border-b border-[#eeebe5] px-4 py-2.5">
            ...
            <div className="flex items-center gap-5 text-[13px] text-slate-500">
              ...
              <select
                value={sortValue}
                onChange={(event) =>
                  setSortValue(event.target.value as SortOptionValue)
                }
          ...
          <aside className="app-panel relative flex w-[340px] flex-col overflow-hidden rounded-2xl bg-white shadow-sm border border-[#f0ece6]">
            <div className="flex items-center gap-2 border-b border-[#f0ece6] px-4 py-3">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  setRows((current) =>
                    current.map((r) =>
                      r.id === selectedRow.id ? { ...r, title: nextTitle } : r,
                    ),
                  );
                }}
            ...
            <div className="flex-1 overflow-auto">
              <div className="grid grid-cols-2 gap-x-5 gap-y-4 border-b border-[#f0ece6] px-4 py-4 text-[13px]">
                ...
                <div>
                  <AvatarPill name={selectedRow.createdBy} />
                </div>
                ...
                <div className="text-slate-700">{selectedRow.creationDate}</div>

                <div className="flex items-center gap-2 text-slate-400">
                  <History className="h-3.5 w-3.5" />
                  <span>Last update</span>
                </div>
                <div className="text-slate-700">{selectedRow.lastUpdate}</div>
              </div>
              ...
              <div className="px-4 py-4">
                ...
                <div className="min-h-[240px] rounded-xl border border-dashed border-[#ece8e1] bg-white p-3 text-[13px] text-slate-500">
                  No additional details provided for this dashboard.
                </div>
              </div>
            </div>
