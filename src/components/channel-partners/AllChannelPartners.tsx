...
type ChannelPartnerView = {
  id: string;
  label: string;
  sort: SortingState;
};

const initialRows: ChannelPartnerRow[] = [];

const views: ChannelPartnerView[] = [
  {
    id: "all",
    label: "All Channel Partner",
...
function AllChannelPartnersPage() {
  ...
  const columns = useMemo<ColumnDef<ChannelPartnerRow>[]>(
    () => [
      {
        id: "select",
        header: () => (
  ...
  function addChannelPartner() {
    ...
    setRows((current) => [
      {
        id: nextId,
        name: `Channel Partner ${current.length + 1}`,
